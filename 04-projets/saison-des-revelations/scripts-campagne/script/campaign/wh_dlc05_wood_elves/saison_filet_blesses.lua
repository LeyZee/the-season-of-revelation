-----------------------------------------------------------------------------------
--	FILET DE SÉCURITÉ DES CHEFS DE FACTION (23.09.2026, 23 h 05 ; chargé par required.lua).
--
...
-- Clés des factions de culture elfe, lues une fois au premier appel (la culture d'une faction ne change pas) : le filet
-- ne parcourt plus toute la liste du monde à la fin du tour de chaque faction (audit de fluidité du 24.09.2026, T1 / S1).
-- Même règle, mêmes moments.
local elfes = nil;
local function factions_elfes()
	if not elfes then
		elfes = {};
		local factions = cm:model():world():faction_list();
		for i = 0, factions:num_items() - 1 do
			local f = factions:item_at(i);
			if f:culture() == CULTURE_ELFE then
				table.insert(elfes, f:name());
			end;
		end;
	end;
	return elfes;
...
local function filet_blesses()
	local liste = factions_elfes();
	for i = 1, #liste do
		local f = cm:get_faction(liste[i]);
		if f and not f:is_dead() and not f:is_human() and not f:has_faction_leader() then
...
-- Audit de fluidité du 25.09.2026, R7 : seulement à la fin du tour d'une faction elfe (et non de chacune des ~30
-- factions) ; une blessure dure plusieurs tours, le seigneur blessé est donc toujours retiré avant son retour.
core:add_listener(
	"saison_filet_blesses",
	"FactionTurnEnd",
	function(context)
		return context:faction():culture() == CULTURE_ELFE;
	end,
	function()
...
out("La Saison des Revelations : filet des chefs en place");
